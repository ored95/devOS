...
static const char *interrupt_name[256] = {
	[INTERRUPT_VECTOR_DIV_BY_ZERO] = "divide by zero",	// DE
	[INTERRUPT_VECTOR_DEBUG] = "debug",
	[INTERRUPT_VECTOR_NMI] = "nmi",
	[INTERRUPT_VECTOR_BREAKPOINT] = "breakpoint",		// BP
	...
	[INTERRUPT_VECTOR_DOUBLE_FAULT] = "double fault",	// DF
	...
	[INTERRUPT_VECTOR_GENERAL_PROTECTION] = "general protection",	// GP
	[INTERRUPT_VECTOR_PAGE_FAULT] = "page fault",		// PF
...
void page_fault_handler(struct task *task)
{
	...
	if ((*pte & PTE_COW) != 0) 	// check COW flag is enable or not
	{
		unsigned perm = *pte & PTE_FLAGS_MASK;		// range 12 bits (0-11) of unusable page
		struct page *new;

		assert((*pte & PTE_P) != 0);
		if ((new = page_alloc()) == NULL) {			// allocate new physical page
			terminal_printf("page_fault_handler: can't allocate page\n");
			goto fail;
		}

		terminal_printf("page fault: va = %p, new page: %p\n", va, new);	
		// map new physical page to virtual page address KERNEL_TEMP
		if (page_insert(task->pml4, new, KERNEL_TEMP, perm | PTE_W) != 0)	// check bit 11 (W) is disabled or not
			goto fail;

		// copy to KERNEL_TEMP original page
		memcpy((void *)KERNEL_TEMP, (void *)ROUND_DOWN(va, PAGE_SIZE), PAGE_SIZE);

		// map new physical page by exception
		if (page_insert(task->pml4, new, ROUND_DOWN(va, PAGE_SIZE), (*pte & PTE_FLAGS_MASK) | PTE_W) != 0)
			goto fail;
		
		// remove map new original physical page by KERNEL_TEMP address
...
void interrupt_init(void)
{
	...
	// LAB4-5 Instruction: initialize idt, don't forget that interrupts and
	// exceptions, which may occur inside kernel space should use IST,
